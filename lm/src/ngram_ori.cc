...
main(int argc, char **argv)
{
	...
	vocab->toLower() = false;
	ngramLM = new Ngram(*vocab, order);  //ÉùÃ÷ºÍ³õÊ¼»¯Ò»¸öngramLM½á¹¹
	assert(ngramLM != 0);
	...
	*/
	...
	//	string templine =pplFile;// "拨打电话10086";
		...
		if (engpro) //need pre text clean
                {
                        string outfile = (string)pplFile + "_engproOut";
                        string cmd;
                        cmd = "python " + subcwd + "//engwordPro.py " + (string)pplFile + " " + outfile;
                        system(cmd.c_str());
                        filename = outfile;
                }
...
//			string id = getlineVec[0];
			string tline  = getline; //getlineVec[1];
...
//			string segprob = pathresultVec[2].c_str();
//			string segprob1 = pathresultVec[1].c_str();
//			string segprob2 = pathresultVec[3].c_str();
//			string segprob3 = pathresultVec[5].c_str();
//			cout<<"segprob:"<<segprob<<" "<<segprob1<<" "<<segprob2<<" "<<segprob3<<endl;
		//	probLM->pplLine((char*)outresult.c_str(), stats);
			//map<string, int>::iterator it;

			//map<string, int> submap = stats.OOVWordsMap;
			//string cmd = "python " + subcwd + "//errorWordCheck.py "+outresult.c_str();
			//system(cmd.c_str());		

			cout <<outresult.c_str()<<endl;// "SegmentResult: \n" << outresult.c_str() << endl;
