...
/*
 * 对待分词文件，读入每个句子，给出每个句子分词结果及句子概率。
 最后输出整个文件中所有句子的合计ppl和概率
 */
unsigned int
		...
		const char *escapeString, Boolean weighted)
{
    ...
    while ((line = file.getline())) {//读入一个句子
	...
	unsigned int numWords =vocab.parseWords(line, sentence, maxWordsPerLine + 1);//得到句子中单词的个数
...
unsigned 
LM::searchSplit(int node, string remainSent,vector< vector<string> >& resultUtt)
{

	int start = node;
	vector< vector<string>  >resultUtt;
	int steplen = 1;
	string curword;
	vector< string >startend;
	while (start + steplen <= remainSent.length())
	{
		curword = remainSent.substr(start, start + steplen);
		if (curword.length > MAXwordlength) //最大词串长度
		{
			break;
		}
		VocabIndex id = vocab.getIndex(curword.c_str());
		if (id == Vocab_None && steplen == 1)  //单词不存在
		{
			startend.push_back(curword);
			startend.push_back(std::to_string(start + steplen));
			resultUtt.push_back(startend);
			break;
		}
		else  //单词存在
		{
...
unsigned 
LM::ngramModelSegment(string inputline,vector<string> pathResult,int order)
{
	...
	char * line; //每行最大字节数，一个字3个字节，如此设定一行最大为10240/3=3413个字，因为长段落，如此设置靠谱
	searchSplit(0, inputline, resultUtt);
	TextStats sentenceStats;
	VocabString* sentenceStart;
	TextStats sentenceStats;
	FloatCount weight;
	for (int i = 0; i < resultUtt.size(); i++)
	{
		strcpy(line, resultUtt[i][0].data());
