/*
 * LM.cc --
 基于ngramModel的分词实现，主要接口
 1.pplFile 	打开待测试文件，给出分词结果
 pplLine 
 2.searchSplit	根据gram中word的列表，对句子做每一次基于节点的切分
 3.ngramModelSegment	基于ngramModel，对每一次切分的串计算概率，保留topk，作为下次切分的基础。继续切分
 *	Generic LM methods
 *
 */
...
#define MAXLENGTHOFWORD 30  //定义一个单词最长的字数，由字典统计而来
 ...
 */
LogP
LM::sentenceProbModi(const VocabString *sentence, TextStats &stats, string endflag)
{
    ...
    LogP prob = sentenceProbModi(sentence,wids, stats,endflag);
 ...
 *	probabilities including that of the sentence end.
 printProb中，储存每个单词得到的概率
 */
LogP
LM::sentenceProbModi(const VocabString *sentence,const VocabIndex *sentenceid, TextStats &myStats,string endflag)
{
  

    string printTemp = "";

    unsigned int len = vocab.length(sentenceid);
    makeArray(VocabIndex, reversed, len + 2 + 1);
    unsigned int i;
	char probtemp[100];  //使用sprintf，将int转为string
    /*
     ...
     */
    len = prepareSentence(sentenceid, reversed, len);
	...
	for (i = len; endflag == "1" ? (int)i >= 0 : (int)i > 0; i--) {
		
	Prob probSum = 0.0;
	myStats.problog += "\tp( " + (string)sentence[len - i] + " | "
		+ (reversed[i + 1] != Vocab_None ? vocab.getWord(reversed[i + 1]) : "")
		+ (i < len ? " ..." : " ") + ") \t= ";
	if (debug(DEBUG_PRINT_WORD_PROBS)) {
	...
	myStats.problog += " " + to_string(LogPtoProb(prob)) + " [ " + to_string(prob) + " ]\n";
	if (debug(DEBUG_PRINT_WORD_PROBS)) {
	...
	if (prob == LogP_Zero) {
		...
		myStats.numOOVs ++;
		myStats.OOVWordsMap[sentence[len - i]] = 1;
	    } else {
		myStats.zeroProbs ++;
		myStats.ZEROWordsMap[sentence[len - i]] = 1;
	...
	//printMap.insert(make_pair(segID, printTemp));
    running(wasRunning);
...
/*
 * 此处为给句子做分词，并给出每个单词的概率，和每个句子的累计概率
 *	
 */
unsigned int
LM::pplLine(char* getline, TextStats &sentenceStats) {
	//char *getline;
	char line[100 * maxWordLength];

	unsigned escapeLen = 0;
	unsigned stateTagLen = 0;
	...
	TextStats weightedStatsFinal;
	string sentenceStartFinal;

	map<int, string> WordIDProbMap;
	Prob finalPPL;
	int segID;
	VocabString* sentenceStart;
	Prob minPPL = INT_MAX;
	string remain = getline;

	strcpy(line, remain.c_str());

	sentNo++;

	unsigned int numWords = vocab.parseWords(line, sentence, maxWordsPerLine + 1);

	if (numWords == maxWordsPerLine + 1)
	{
		dout() << "too many words per sentence\n";
		...
		LogP prob = sentenceProbModi(sentenceStart, sentenceStats, "");
		...
		if (denom > 0)
		{
			finalPPL = LogPtoPPL(sentenceStats.prob / denom);
		}
		sentenceStats.sentlog += to_string((int)sentenceStats.numSentences) + " sentences, "
			+to_string((int)sentenceStats.numWords) + " words, "
			+to_string((int)sentenceStats.numOOVs) +" OOVs\n";

		if (sentenceStats.numWords + sentenceStats.numSentences > 0) {
			sentenceStats.sentlog += to_string((int)sentenceStats.zeroProbs) + " zeroprobs, ";

			// set precision for LogP-based values following
			sentenceStats.sentlog += "logprob= " + to_string(sentenceStats.prob);

			double denom = sentenceStats.numWords - sentenceStats.numOOVs - sentenceStats.zeroProbs
				+ sentenceStats.numSentences;

			if (denom > 0) {
				sentenceStats.sentlog += " ppl= " + to_string(LogPtoPPL(sentenceStats.prob / denom));
			}
			else {
				sentenceStats.sentlog += " ppl= undefined";
			}
		}
		return totalWords;
	}
...
{
	char *getline;
	char line[100 * maxWordLength]; //Ã¿ÐÐ×î´ó×Ö½ÚÊý£¬Ò»¸ö×Ö3¸ö×Ö½Ú£¬Èç´ËÉè¶¨Ò»ÐÐ×î´óÎª10240/3=3413¸ö×Ö£¬ÒòÎª³¤¶ÎÂä£¬Èç´ËÉèÖÃ¿¿Æ×
	...
	while ((getline = file.getline()))
	{
		...
		//cout << "--------------------------match dict end! -----------" << endl;
		dout() << weightedStatsFinal << endl;
...
//判断输入的句子。是否包含英文。将句子中中文和英文劈开
//eg： 我爱NBA直播锦标赛，需要变成 我爱 NBA  直播锦标赛 ，三个string到vector中
//然后中文传递给分词接口，英文保留不处理，每个string挨个做分词，最后输出再将三个结果连接起来
void
LM::checkEngword(string inputline, vector<pair<string, int>> &wordcodeCheckV)
{
	if (inputline.substr(inputline.length() - 1, 1) == "\n")
		...
		for (int i = 0; i<s.length(); i++)
		{
			...
			{
				if (hanzit.length() != 0) //已经存在中文，先导出中文
				{
					wordcodeCheckV.push_back(make_pair(hanzit, 1));
					hanzit = "";
				}
				engt.append(s.substr(i, 1));
			}
			else//<0,>255的是汉字,处理两个字节
			{
				if (engt.length() != 0) //已经存在eng文，先导出eng文
				{
					wordcodeCheckV.push_back(make_pair(engt, 2));
					engt = "";
				}
				hanzit.append(s.substr(i, 3));
				//hanzit.append(s.substr(i, 2));
				//hanzit.append("/");
				++i;
				++i;
			}
...
void
LM::searchSplit(int node, string remainSent, vector<pair<string,int> > &resultUttVP)
{
	...
	string otherword = "";
	int engflag = 0;
	vector< string >startend;  //调低3档
	while (outnode < remainSent.length())
	{
		string tempword = remainSent.substr(start+engstep , 1);  //检测是否有数字或者英文字母的存在
	
		if (tempword[0] < 255 && tempword[0]>0)
		{
			if (tempword == " ") //一个英文单词末尾的空格
			{
				if (otherword != "")
				{
					resultUttVP.push_back(make_pair(otherword, outnode+1));
					otherword = "";
					break;
				}
			}
		...
		{
			resultUttVP.push_back(make_pair(otherword,outnode));
			break;
		}
		curword = remainSent.substr(start, steplen*3);
		outnode =start+ steplen * 3;
		if (curword.length() > MAXLENGTHOFWORD) //最大词串长度
		{
			break;
		}
		VocabIndex id = vocab.getIndex(curword.c_str());
		if (id == Vocab_None)
		{
			if (steplen == 1)  //单词不存在，按找单字加入
			{
				resultUttVP.push_back(make_pair(curword, start + (steplen*3)));
...
vector<string> 
LM:: split(const string& src, const string& separator)
{
	vector<string> dest;
	string str = src;
	string substring;
...
bool judge(const pair<string, double> a, const pair<string, double> b)
{
	if (a.second == b.second)return a.first<b.first;
	else return a.second>b.second;  //
}

bool judgestring(const pair<string, string> a, const pair<string, string> b)
{
	if (a.second == b.second)return a.first<b.first;
	else return atof(a.second.c_str())> atof(b.second.c_str());  //
}

/*
计算单词的概率，不同的order设定不同的context
入参的wordflag代表句首，句末还是句中。如果当前词是句首，context是<s>，如果是句末，prob应该是curword+</s>
step1:劈分前文，根据不同order，获取不同context
step2：context写入reversed结构中。，转为vocabindex。
step3：判断wordflag，如果是末尾。补充处理
*/
LogP
LM::wordprobforSeg(string preWord,string curWord,int order,int wordflag,
	string &problog,int &numOOVs,int &zeroProbs)
{
	string contextPre = ""; string contextPrePre = "";
	LogP probend = 0.0;
	string templog = "";
	LogP prob = 0.0;
	//cid = (VocabIndex*)malloc(sizeof(VocabIndex));
	//memcpy(cid, 0, sizeof(VocabIndex));
	makeArray(VocabIndex, cid, 1+ 2 + 1);
	makeArray(VocabIndex, eid, 1 + 2 + 1);//保存最后一个单词的context
	vector<string> wordsplitVec = split(preWord, " ");
	int vsize = wordsplitVec.size();
	if (vsize < 2) //前面就是 <s>
	{
		contextPre = preWord;
		cid[0] = vocab.getIndex((VocabString)contextPre.c_str());
		cid[1] = Vocab_None;

		eid[0] = vocab.getIndex((VocabString)curWord.c_str());
		eid[1] = Vocab_None;
		
	}
	if (vsize ==2) //前面是一个字，那就是句子开头 <s> + preword
	{
		contextPrePre = Vocab_SentStart;
		contextPre = wordsplitVec[1];
		cid[1] = vocab.getIndex((VocabString)contextPrePre.c_str());// vocab.getIndex((VocabString)contextPrePre.c_str());
		cid[0] = vocab.getIndex((VocabString)contextPre.c_str());
		cid[2] = Vocab_None;
	...
	{
		contextPrePre = wordsplitVec[wordsplitVec.size() - 2];
		contextPre = wordsplitVec[wordsplitVec.size() - 1];
	...
	prob = wordProb(vocab.getIndex((VocabString)curWord.c_str(), vocab.unkIndex()), cid);
	problog += "\n\tp( " + curWord + " | " + contextPre + ") \t= " + " "
		+ to_string(LogPtoProb(prob))
		+ " [ " + to_string(prob) + " ]";

	if (wordflag == 3)//add end flag </s>
	{
		
			
		probend = wordProb(vocab.getIndex((VocabString)Vocab_SentEnd), eid);  //p(</s>/word1,word2)的概率
		templog = "\n\tp( </s> | " + curWord + ") \t= " + " "
			+ to_string(LogPtoProb(probend))
			+ " [ " + to_string(probend) + " ]";
	}
	
	if (prob == LogP_Zero) {
	
		if (vocab.getIndex((VocabString)curWord.c_str()) == vocab.unkIndex() || vocab.getIndex((VocabString)curWord.c_str())==-1)
		{
			problog += " [OOV]";
			numOOVs++;
		}
...
unsigned
LM::ngramModelSegment(string inputline, int order, vector<pair<string, string>> &pathCanWordVP, 
	int beamsize)
{
	if (inputline.substr(inputline.length() - 1, 1) == "\n")
		inputline = inputline.substr(0, inputline.length() - 1);
	vector<pair<string,int> > resultUttVP;
	VocabString sentence[maxWordsPerLine + 1];
	vector<pair<string, double>> segWordProbVP;
	vector<string> getsequence;
	string jointemp;
	float detaprob =0.435;// 0.585;//0.485;   //0.585 //0.385;
	
	int wordflag = 0;
	int numOOVs = 0;
	int zeroProbs = 0;

	char line[100 * maxWordLength];//每行最大字节数，一个字3个字节，如此设定一行最大为10240/3=3413个字，因为长段落，如此设置靠谱
	//首先添加第一个单词的遍历结果，结果在resultUtt中
	
	//cout<<"inputline:" <<inputline<<endl;
	searchSplit(0, inputline, resultUttVP);

	if (resultUttVP.size() == 0)  //分割结果是空的
		return 0;
	//先给出第一个单词的分割结果
	string preword = Vocab_SentStart;
	string problog = "";
	int wordnum = 1;
	for (int i = 0; i < resultUttVP.size(); i++)
	{
		problog = "";
		wordflag = 1;
		strcpy(line, resultUttVP[i].first.data());
		string getword = resultUttVP[i].first.c_str();
	//	cout<<"getword:"<<getword<<endl;
		VocabString curword = (VocabString)resultUttVP[i].first.c_str();		if((getword.length() == inputline.length()))
		{
			wordflag = 3;  //倒数第二个词，需要考虑后缀</s>
		}
		LogP prob = wordprobforSeg(preword, curword, order, wordflag, problog,numOOVs,zeroProbs);
		
		prob = (prob == LogP_Zero ? 0.0 : (prob -detaprob));
	//	prob = (prob == LogP_Zero ? 0.0 : prob);
	//	cout<<"word:"<<curword<<"\toriprob:"<<prob<<endl;
	//	prob = prob- 0.385;
	//	cout<<"aftermodiProb:\t"<<prob<<endl;
		jointemp = preword+ " "+resultUttVP[i].first + "_" + to_string(resultUttVP[i].second)+ "_"+problog+"_"+to_string(wordnum)+"_"+to_string(numOOVs)+"_"+to_string(zeroProbs);
		segWordProbVP.push_back(make_pair(jointemp, prob)); //key为当前单词+单词索引位置，value为对应prob
		
	}

	sort(segWordProbVP.begin(), segWordProbVP.end(), judge);
	vector<pair<string,double>>::iterator it;
	for (int i = 0; i<segWordProbVP.size(); i++)
	{
		if (i >= beamsize)
			break;
		jointemp = segWordProbVP[i].first + "_" + to_string(segWordProbVP[i].second);
		getsequence.push_back(jointemp);

	}
	
	string endnode, preprob;
	int printflag = 0;
	//第一个单词后面单词的结果
	while (true)
	{
	//	cout<<"size:"<<getsequence.size()<<endl;
		if (getsequence.size()== 0)
			break;
		segWordProbVP.clear();
	//	cout << "getsequence start!" << endl;
		for (int i = 0; i < getsequence.size(); i++)
		{
			
			jointemp = getsequence[i];
	//		cout<<"jointemp:"<<jointemp<<endl;	
			vector<string> splittemp;
			splittemp = split(jointemp, "_");
			preword = splittemp[0];
			endnode = splittemp[1];
			problog = splittemp[2];
			wordnum = atoi(splittemp[3].c_str());
			numOOVs = atoi(splittemp[4].c_str());
			zeroProbs = atoi(splittemp[5].c_str());
			preprob = splittemp[6];

			if (atoi(endnode.c_str()) >= inputline.length())
			{
	//			cout << "endsent,preword :" << preword << " preprob:" << preprob << endl;
				pathCanWordVP.push_back(make_pair(preword+"_"+problog+"_" + to_string(wordnum)+"_"+to_string(numOOVs)+"_"+to_string(zeroProbs), preprob));
				printflag = 1;
				continue;
			}
			
			vector<pair<string, int> > secResultUttVP;
			searchSplit(atoi(endnode.c_str()), inputline, secResultUttVP);
			if (secResultUttVP.size() == 0) return 0;
			string cantemp = "";
			string endflag = "";
			//out << "segresult start!" << endl;
			for (int k = 0; k < secResultUttVP.size(); k++)
			{
				string sublog = "";
				cantemp = "";
				string candidateWord = secResultUttVP[k].first;
				if (candidateWord.substr(0, 1) == " ")
					candidateWord = candidateWord.substr(1, inputline.length());
				if ((candidateWord.length() + atoi(endnode.c_str()) == inputline.length() ))
				{
					wordflag = 3;  //倒数第二个词，需要考虑后缀</s>
				}
				LogP getprob = wordprobforSeg(preword, candidateWord, order, wordflag, sublog,numOOVs,zeroProbs);
					
			//        LogP sentprob = atof(preprob.c_str()) + (getprob ==LogP_Zero ? 0.0 : getprob);
				//cout<<"curword:"<<candidateWord<<"\toriprob:"<<sentprob<<endl;
				LogP sentprob = atof(preprob.c_str()) + (getprob ==LogP_Zero ? 0.0 : (getprob -detaprob));
			        //sentprob = sentprob - 0.385;
				//cout<<"aftermodiProb:\t"<<sentprob<<endl;
				jointemp =preword+" "+candidateWord+ "_" + to_string(secResultUttVP[k].second)
					+ "_"+problog+sublog+"_"+to_string(wordnum+1)+"_"+to_string(numOOVs)+"_"+to_string(zeroProbs);
				segWordProbVP.push_back(make_pair(jointemp, sentprob)); //key为当前单词+单词索引位置，value为对应prob
	...
	if (pathCanWordVP.size() == 0)
		cout << "error!not get the segresult from :\t" << inputline.c_str() << endl;
	//cout<<"start sort\n";
	sort(pathCanWordVP.begin(), pathCanWordVP.end(), judgestring);
	//cout<<"end sort\n";
	return 1;
}
...
int  LM::MatchDict(VocabString head, string remain, vector<string>& splitUtt)
{
	string head2;
	for (int i = 3; i < maxWordLength; i += 3)	
		
	{
		if (remain.length() >= i)
		{
			string head1 = remain.substr(0, i).c_str();

			VocabIndex id = vocab.getIndex(head1.c_str());   //´Ê±íÖÐÕÒÒ»×Ö
			if (id != Vocab_None)  
			{
				head2 =(string)head + " " + head1;
				string remain1 = remain.substr(i, remain.length());  //ÓàÏÂµÄ´ÊÓï
																	 //cout<< head2.c_str()<<"    "<< remain1.c_str()<<endl;   
				int ret = MatchDict(head2.c_str(), remain1, splitUtt);
				if (UTT_END == ret)
				{
		...
		else if (remain.length() == 0)	//Ö±µ½´Ë´Î±éÀú×ßÍê£¬ÓàÏÂÎª¿Õ£¬´ú±íheadºóÆ´½ÓµÄ´ÊÓïÎ´ÕÒµ½£¬ÔòheadÎªÄ¿Ç°×î´ó´Ê´®¡£Ð´Èë
		{
			splitUtt.push_back(head);  //½«head¸³Öµ¸øresultstr
			return UTT_END;
		}
		else
		{
