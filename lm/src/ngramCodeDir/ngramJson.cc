	...
	*/
	...
	if (pplFile)
	{
		...
		while ((getline = file.getline()))
		{
			...
			/*for (int i = 0; i < pathCanWordVP.size(); i++)
			{
				cout << pathCanWordVP[i].first.c_str() << " " << pathCanWordVP[i].second.c_str() << endl;
			}*/
			...
			OOVWords += pathresult.OOVWord;
			lineOOV = useLM->split(pathresult.OOVWord, ",");
			zeroWords += pathresult.lowProbWord;
			...
			//{
				...
				/*jsonItem["0SegmengResult"] = outresult.c_str();
				jsonItem["1OOVWords"] = pathresult.OOVWord; //添加数据
				jsonItem["2ZerosProbWords"] = pathresult.lowProbWord.c_str();
				jsonItem["3ProbLog"] = wordproblog.c_str();
				jsonItem["4SentLog"] = sentlog;
				jsonRoot.append(jsonItem);
				cout << jsonRoot.toStyledString() << endl; //输出到控制台*/

				cout << "\t\t" << "\"" << "4SentLog" << "\"" << " : " << "\"" << sentlog.c_str() << "\"" << endl;
				cout << "\t}\n]" << endl;
