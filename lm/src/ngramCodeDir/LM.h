...
public:
    ...
    virtual LogP sentenceProbModi(const VocabString *sentence,const VocabIndex *sentenceid, TextStats &stats, string endflag );
    virtual LogP sentenceProbModi(const VocabString *sentence, TextStats &stats, string endflag);
	virtual vector<string> split(const string& src, const string& separator);
	...
	virtual void  checkEngword(string inputline, vector<pair<string, int>> &wordcodeCheckV);
	...
	virtual unsigned pplLine(char* getline, TextStats &stats);
	virtual int MatchDict(VocabString head, string remain, vector<string>& splitUtt);
	virtual void searchSplit(int node, string remainSent, vector<pair<string, int> >  &pathResultVP);
	virtual unsigned int ngramModelSegment(string inputline, int order, vector<pair<string, string>> &pathCanWordVP,
		int beamsize);
	virtual LogP wordprobforSeg(string preWord, string curWord, int order, 
		int wordflag, string &problog, int &numOOVs, int &zeroProbs);
