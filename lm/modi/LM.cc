...
/*
* ½«¾ä×Ó¸ù¾ÝngramÖÐµÄ´Ê±í²é´Ê×ö·Ö´Ê
*/
 ...
 */
LogP
LM::sentenceProb(const VocabString *sentence, TextStats &stats,vector<string> &printVec)
{
    ...
    LogP prob = sentenceProb(wids, stats,printVec);
 ...
 */
LogP
LM::sentenceProb(const VocabIndex *sentence, TextStats &stats,vector<string> &printProbVec)
{
    TextStats myStats;
	...
	if (debug(DEBUG_PRINT_WORD_PROBS)) {
		printTemp.append(vocab.getWord(reversed[i]));
		printTemp.append(" | ");
		printTemp.append((reversed[i + 1] != Vocab_None ? vocab.getWord(reversed[i + 1]) : ""));
		printTemp.append((i < len ? " ..." : " "));
		printTemp.append(") \t= ");
	...
	if (debug(DEBUG_PRINT_WORD_PROBS)) {
 		printTemp +=" " + to_string(LogPtoProb(prob)) + " [ " + to_string(prob) + " ]";
 		printProbVec.push_back(printTemp);
 ...
 */
unsigned int
LM::pplFile(char* getline, TextStats &stats, string &sentSegResult, vector<string> &WordProbVec,
		const char *escapeString, Boolean weighted)
{
	...
	//{
		...
		remain = remain.substr(0, remain.length() - 1);//È¥µôÄ©Î²µÄ\n
//分词入口，MatchDict
		//cout << "==========Start MatchDict========" << endl;
		MatchDict("", remain, splitUtt);
		//cout << "match dict end,get resultstr:  " << endl;
		for (int i = 0; i < splitUtt.size(); i++)
		{
			strcpy(line, splitUtt[i].data());
//ÒÔÉÏ²¿·ÖÎªÌí¼Ó²¿·Ö
			...
			*/
			...
			if (numWords == maxWordsPerLine + 1)
			{
				...
				*/
				...
				LogP prob = sentenceProb(sentenceStart, sentenceStats,WordProbVec);
			...
			{
				minPPL = finalPPL;
				weightedStatsFinal = sentenceStats;
				sentenceStartFinal = splitUtt[i].data();
			}
		...
		//cout << "The best segment result is : ";
		dout() << sentenceStartFinal << endl;
		dout() << weightedStatsFinal << endl;
		sentSegResult.append(sentenceStartFinal + "\n");
		//sentSegResult.append(weightedStatsFinal);
...
{
	string head2;
	for (int i = 3; i < maxWordLength; i += 3)	//Ã¿´ÎµÝ½øÒ»¸öºº×Ö£¬3¸ö×Ö½Ú,maxlenÎªÒ»¸ö´Ê´®µÄ×î´ó³¤¶È
	{
		if (remain.length() >= i)
		{
			...
			if (id != Vocab_None)  //·µ»ØµÄindexÊÇÎÞÐ§index£¬Ö¤Ã÷Ã»ÕÒµ½
			{
